...
import org.jeecg.common.api.dto.LoginSkipUserDTO;
...
public class LoginController {
	...
	@ApiOperation("生成登录子系统token")
	@GetMapping("/generateKey")
	public Result<String> generateKey(LoginSkipUserDTO user){
		String token = JwtUtil.sign(user.getUsername() + "-*-" +user.getAppId(), user.getPassword());
		return Result.ok(token);
	}

	@ApiOperation("校验子系统token")
	@GetMapping("/verifyKey")
	public Result<String> verifyKey(LoginSkipUserDTO user){
		if (JwtUtil.verify(user.getToken(), user.getUsername() + "-*-" +user.getAppId(), user.getPassword())){
			return Result.ok(null);
		}
		return Result.error("token fail");
	}
